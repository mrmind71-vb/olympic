...
cStr1 = " SELECT FILE1_10.GROUP, FILE1_10.DESCA, FILE1_10.ITEM, Sum(FILE1_11.[IN]) AS SumIN, Sum(FILE1_11.OUT) AS SumOUT FROM FILE1_10 LEFT JOIN FILE1_11 ON FILE1_10.ITEM = FILE1_11.ITEM where file1_10.item is not null "
If xStore1.BoundText <> "" Then cStr1 = cStr1 & " and STORE   = " & MyParn(xStore1.BoundText)
If IsDate(xdate1.Text) Then cStr1 = cStr1 & " and Date < " & DateSql(xdate1.Text)
cStr1 = cStr1 & " GROUP BY FILE1_10.GROUP, FILE1_10.DESCA, FILE1_10.ITEM ORDER BY FILE1_10.ITEM"
Set itemTable = mydb.OpenRecordset(cStr1)

cStr1 = " SELECT  Sum(FILE1_11.[IN]) AS SumIN, Sum(FILE1_11.OUT) AS SumOUT , ITEM FROM FILE1_11 where file1_10.item is not null "
If xStore1.BoundText <> "" Then cStr1 = cStr1 & " and STORE   = " & MyParn(xStore1.BoundText)
If IsDate(xDate2.Text) Then cStr1 = cStr1 & " and Date <= " & DateSql(xDate2.Text)
cStr1 = cStr1 & " GROUP BY FILE1_11.ITEM ORDER BY ITEM"
Set ITEM2table = mydb.OpenRecordset(cStr1)

cString = "SELECT FILE1_60.STORE1, FILE1_60.item, Sum(FILE1_60.QUANT) AS SumTransFrom " & _
          "FROM FILE1_60 where file1_60 " & _
          " where Date Between DateValue(" & MyParn(xdate1.Text) & ")" & _
          " and DateValue(" & MyParn(xDate2.Text) & ")" & _
          " and STORE1 = " & MyParn(xStore1.BoundText) & _
          " GROUP BY FILE1_60.STORE1, FILE1_60.item "
