...
Do Until GRDTABLE.EOF
    grid1.AddItem ""
    grid1.TextMatrix(grid1.Rows - 1, 0) = GRDTABLE(mySplit(myPublic(1))) & ""
    grid1.TextMatrix(grid1.Rows - 1, 1) = GRDTABLE(myPublic(2)) & ""
    grid1.TextMatrix(grid1.Rows - 1, 2) = GRDTABLE(mySplit(myPublic(3))) & ""
    GRDTABLE.MoveNext
Loop
grid1.AddItem ""
End Sub
...
Dim aInsert(2, 1) As String
aInsert(0, 0) = mySplit(myPublic(1))
If mySplit(myPublic(1), 2) = "n" Then
...
aInsert(2, 0) = mySplit(myPublic(3))
If mySplit(myPublic(3), 2) = "n" Then
...
If mySplit(myPublic(1), 2) = "n" Then
    If Not IsNumeric(grid1.TextMatrix(Row, 0)) Then
        grid1.TextMatrix(Row, 0) = addvalue(Newflag(myPublic(0), mySplit(myPublic(1))))
        aInsert(0, 1) = addvalue(grid1.TextMatrix(Row, 0))
        con.Execute CreateInsert(aInsert, myPublic(0))
    Else
           con.Execute CreateUpdate(aInsert, myPublic(0), " WHERE " & Split(myPublic(1)) & " = " & grid1.TextMatrix(Row, 0))
    End If
Else
    If Trim(grid1.TextMatrix(Row, 0)) = "" Then
        grid1.TextMatrix(Row, 0) = RetZero(Newflag(myPublic(0), mySplit(myPublic(1))), mySplit(myPublic(1), 3))
        aInsert(0, 1) = addstring(grid1.TextMatrix(Row, 0))
        con.Execute CreateInsert(aInsert, myPublic(0))
    Else
        con.Execute CreateUpdate(aInsert, myPublic(0), " WHERE " & mySplit(myPublic(1)) & " = " & MyParn(grid1.TextMatrix(Row, 0)))
    End If
    ...
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Õ–›?? Â· √‰  „Ê«›ﬁ", vbYesNo + vbCritical, "Õ–›") = vbYes Then
            If IsNumeric(grid1.TextMatrix(grid1.Row, 0)) Then
                ...
                Else
                    con.Execute "Delete From " & myPublic(0) & " where code =  " & MyParn(grid1.TextMatrix(grid1.Row, 0))
                End If
                con.CommitTrans
            End If
...
.ColWidth(1) = 2000
.ColWidth(2) = 2000
.ColComboList(2) = myPublic(4)
.TextMatrix(0, 0) = myPublic(5)
.TextMatrix(0, 1) = myPublic(6)
.TextMatrix(0, 2) = myPublic(7)

Me.Caption = myPublic(7)
