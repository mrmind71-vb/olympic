...
cDir = Mid(MdbPath, 1, nLast - 1)
cFileData = Mid(MdbPath, nLast + 1)
cFilezip = Replace(LCase(cFileData), ".mdb", "") & RetZero(Day(Date)) & ".zip"
Set fs = CreateObject("Scripting.FileSystemObject")
aDir(0) = "D:\back"
aDir(1) = "e:\back"
aDir(2) = "f:\back"
...
Private Sub RSPZip1_Finished(ReturnCode As Long, ReturnDescription As String)
    ...
    For i = 1 To UBound(aDir)
        ...
        fs.CopyFile aDir(0) & "/" & cFilezip, aDir(i) & "/" & cFilezip
    Next
    MsgBox " „ «·‰”Œ »‰Ã«Õ"
