...
If nYears > nMax_Years Then
    MsgBox "⁄œœ «·„Ê«”„ «ﬂ»— „‰ " & nMax_Years & " „Ê«”„"
End If
...
End Function
Public Function addInstall(pCode As String, pDate As String, pCon As ADODB.Connection, Optional ByVal pDoc_No As String) As Variant
Dim cString As String, sDoc_no As String
Dim nLate As Long
If Not ValidNum(pDoc_No) Then
    cString = "SELECT TOP 1 ID,dbo.f_install_rest(file6_21.ID) AS REST,FILE6_21.RATE_TAX,FILE6_21.TAX_DIFF FROM FILE6_21 WHERE FILE6_21.CODE = " & pCode & _
...
If Not IsEmpty(aRet) Then
    ...
    aInsert = AddFlag(aInsert, "late_id", retFlag(aRet, "ID"))
    aInsert = AddFlag(aInsert, "value", mRound(retFlag(aRet, "REST")))
    aInsert = AddFlag(aInsert, "tax_rate", retFlag(aRet, "RATE_TAX"))
    aInsert = AddFlag(aInsert, "tax_diff", retFlag(aRet, "TAX_DIFF"))
