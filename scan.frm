   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin Threed.SSCommand cmdScan 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "scan.frx":2765
         ButtonStyle     =   3
         PictureAlignment=   11
         BevelWidth      =   0
         PictureDisabledFrames=   1
         ShapeSize       =   1
         PictureDisabled =   "scan.frx":4C6D
      End
      ...
      Begin Threed.SSCommand cmdFile 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "scan.frx":6EEE
         ButtonStyle     =   3
         PictureAlignment=   11
         BevelWidth      =   0
         PictureDisabledFrames=   1
         ShapeSize       =   1
         PictureDisabled =   "scan.frx":9133
      End
...
If Not validPhoto(RetPhoto(aPhoto(Index - 1, 2))) Then
    MsgBox "·«  ÊÃœ ’Ê—…"
    Exit Sub
End If
If MsgBox("”Ì „ «·«‰ Õ–› ’Ê—… «·⁄÷Ê ", vbOKCancel + vbDefaultButton2, "Õ–› «·’Ê—…") = vbCancel Then Exit Sub
Photo1(Index).Images.Clear
If fs.FileExists(RetPhoto(aPhoto(Index - 1, 2))) Then fs.DeleteFile RetPhoto(aPhoto(Index - 1, 2))
Inform " „ Õ–› «·’Ê—… »‰Ã«Õ"
Handlecontrols
End Sub
...
If GetDesca("select code from file1_10 where code = " & cStr2) = "" Then
    MsgBox "·« ÌÊÃœ ⁄÷Ê »Â–« «·—ﬁ„"
    Exit Sub
End If

If fs.FileExists(RetPhoto(cStr1)) Then If MsgBox(" ÊÃœ ’Ê—… »‰›” «·≈”„ !! «” »œ«· «·’Ê—… «·Õ«·Ì… „ﬂ«‰Â«", vbOKCancel + vbDefaultButton2) = vbCancel Then Exit Sub

If MsgBox("”Ì „ «·«‰ ‰ﬁ· «·’Ê—… «·Ì «·⁄÷Ê " & cStr1, vbOKCancel + vbDefaultButton1, "‰ﬁ· «·’Ê—…") = vbCancel Then Exit Sub
Photo1(Index).Export.ToFile RetPhoto(cStr1), ixfsJPG
If fs.FileExists(RetPhoto(aPhoto(Index - 1, 2))) Then fs.DeleteFile RetPhoto(aPhoto(Index - 1, 2))
Photo1(Index).Images.Clear
Exit Sub
myerror:
...
If Not Photo1(Index).IsEmpty Then
    Photo1(Index).Export.ToFile RetPhoto(aPhoto(Index - 1, 2)), ixfsJPG
    Inform " „ Õ›Ÿ « ·„·›"
End If
Exit Sub
myerror:
...
Photo1(Index).Images.Clear
If validPhoto(RetPhoto(aPhoto(Index - 1, 2))) Then
    Photo1(Index).Import.FromFile RetPhoto(aPhoto(Index - 1, 2))
End If
End Sub
...
On Error GoTo myerror
If WrongFile(RetPhoto(aPhoto(Source.Index - 1, 2))) Then Exit Sub
If WrongFile(RetPhoto(aPhoto(Index - 1, 2))) Then Exit Sub
...
If Photo1(Index).IsEmpty Then
    ...
    Handlecontrols
    If fs.FileExists(RetPhoto(aPhoto(Source.Index - 1, 2))) Then fs.DeleteFile RetPhoto(aPhoto(Source.Index - 1, 2))
...
cString = "Select file1_11.member, file1_11.code,file1_11.desca,file1_11.title,relation_codes.desca as rel_Desca  From File1_11 left join relation_codes on file1_11.relation = relation_codes.code where file1_11.Member = " & sCode & " order by code"
...
aPhoto(0, 0) = "⁄÷Ê «”«”Ì"
aPhoto(0, 1) = GetField("select desca from file1_10 where code = " & sCode)
aPhoto(0, 2) = sCode
...
'    cmdUndo(i2).Enabled = Photo1(i2).IsChanged And Not Photo1(i2).IsEmpty
    ...
    cmddel(i2).Enabled = fs.FileExists(RetPhoto(aPhoto(i2 - 1, 2)))
    
    'cmdFile(i2).Enabled = Photo1(i2).IsEmpty
    'cmdScan(i2).Enabled = Photo1(i2).IsEmpty
    
    If fs.FileExists(RetPhoto(aPhoto(i2 - 1, 2))) And Not validPhoto(RetPhoto(aPhoto(i2 - 1, 2))) Then
    ...
    End If
    cmdMove(i2).Enabled = validPhoto(RetPhoto(aPhoto(i2 - 1, 2)))
Next
End Sub
