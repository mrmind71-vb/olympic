...
Begin VB.Form Notes_codesfrm 
   ...
   End
End
Attribute VB_Name = "Notes_codesfrm"
...
cString = "SELECT NOTES_CODES.CODE as [«·ﬂÊœ],NOTES_CODES.DESCA as [—ﬁ„ «·œ› —],NOTES_CODES.BON AS [«Ê· »Ê‰],NOTES_CODES.BON_COUNT AS [⁄œœ «·»Ê‰« ],NOTES_CODES.QUANT AS [«·ﬂ„Ì…],TYPE AS [«·‰Ê⁄]" & _
          " FROM NOTES_CODES"
If Trim(xDesca.Text) <> "" Then
    cString = cString & turn(cString) & MyParnAnd(xDesca.Text, "NOTES_CODES.desca")
End If
If Val(xBon.Text) <> 0 Then
    cString = cString & turn(cString) & "BON <= " & Val(xBon.Text) & " AND " & "BON_LAST >= " & Val(xBon.Text)
End If
cString = cString & " ORDER BY NOTES_CODES.CODE"
...
openCon con
cList = StrList2("SELECT CODE,DESCA FROM TYPE_GAS_CODES ORDER BY CODE")
...
'        cString = "Select code from NOTES_CODES where desca = " & MyParn(grid1.EditText)
...
ElseIf Trim(grid1.EditText) = "" Then
    MsgBox grid1.TextMatrix(0, Col) & " €Ì— „”Ã·"
    Cancel = True
End If
End Sub
Private Sub Fixgrd()
...
Private Sub xBon_no_Change()

End Sub

Private Sub xBon_Change()
myload
End Sub
...
If Not ValidInt(grid1.TextMatrix(Row, 3)) Then Exit Function
If Not ValidInt(grid1.TextMatrix(Row, 4)) Then Exit Function
If Not ValidInt(grid1.TextMatrix(Row, 5)) Then Exit Function
validRow = True
End Function
...
If KeyCode = 46 And grid1.Row <> grid1.Rows - 1 Then
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Õ–› !! „Ê«›ﬁ", vbOKCancel + vbDefaultButton2) = vbOK Then
            con.BeginTrans
            On Error GoTo myerror
            con.Execute "delete from NOTES_CODES where CODE = " & MyParn(grid1.TextMatrix(grid1.Row, 0))
            con.CommitTrans
...
If KeyCode = 13 And Col <> 5 Then CellPos KeyCode, Row, Col
End Sub
...
aInsert = AddFlag(aInsert, "[BON]", addstring(grid1.TextMatrix(Row, 2)))
aInsert = AddFlag(aInsert, "[BON_COUNT]", Val(grid1.TextMatrix(Row, 3)))
aInsert = AddFlag(aInsert, "[QUANT]", Val(grid1.TextMatrix(Row, 4)))
aInsert = AddFlag(aInsert, "[TYPE]", addvalue(grid1.TextMatrix(Row, 5)))
con.BeginTrans
On Error GoTo myerror
If grid1.TextMatrix(Row, 0) = "" Then
    Dim sCode As String
    sCode = Newflag("NOTES_CODES", "CODE", con)
    aInsert = AddFlag(aInsert, "[CODE]", sCode)
    con.Execute addInsert(aInsert, "NOTES_CODES")
Else
    con.Execute addUpdate(aInsert, "NOTES_CODES", "NOTES_CODES.CODE = " & grid1.TextMatrix(Row, 0))
End If
con.CommitTrans
myreplace = True
Exit Function
