...
aInsert = AddFlag(aInsert, "[DEGREE]", addvalue(grid1.TextMatrix(Row, 2)))
aInsert = AddFlag(aInsert, "[TYPE]", addstring(grid1.TextMatrix(Row, 3)))
aInsert = AddFlag(aInsert, "[SPC]", addstring(grid1.TextMatrix(Row, 4)))

On Error GoTo myerror
con.BeginTrans
If grid1.TextMatrix(Row, 0) = "" Then
    grid1.TextMatrix(Row, 0) = Newflag("CLASS_CODES", "CODE")
    aInsert = AddFlag(aInsert, "[CODE]", addvalue(grid1.TextMatrix(Row, 0)))
    con.Execute addInsert(aInsert, "CLASS_CODES")
Else
    con.Execute addUpdate(aInsert, "CLASS_CODES", "CODE = " & grid1.TextMatrix(Row, 0))
End If
con.CommitTrans
Exit Sub
myerror:
