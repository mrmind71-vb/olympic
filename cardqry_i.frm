...
Begin VB.Form cardqry_ifrm 
   ...
   Begin VB.Frame Frame8 
      ...
      End
   End
   Begin Threed.SSCommand cmdAddUnpaid 
      Height          =   645
      Left            =   3510
      TabIndex        =   40
      Top             =   1035
   ...
   End
End
Attribute VB_Name = "cardqry_ifrm"
...
checkPhoto
End Sub
Private Sub cmdAddUnpaid_Click()
myLoadGrdPaid
On Error Resume Next
grid1.SaveGrid cFileSave, flexFileData
Err.Clear
...
For i = 1 To grid1.rows - 1
    If Not validPhoto(RetPhoto_I(grid1.TextMatrix(i, 2))) Then
...
Dim i As Long
For i = 1 To grid1.rows - 1
    If Not validPhoto(RetPhoto_I(grid1.TextMatrix(i, 0))) Then
        grid1.RowHidden(i) = True
    End If
Next
...
If Row = 0 And Col = grid1.Cols - 1 Then
    With grid1
    For i = 1 To .rows - 1
        .TextMatrix(i, Col) = IIf(.Cell(flexcpChecked, 0, Col) = 1, -1, 0)
    Next
    End With
End If
grid1.SaveGrid cFileSave, flexFileData
...
grid1.Editable = IIf(grid1.Col = grid1.Cols - 1 Or grid1.Col = 11, flexEDKbdMouse, flexEDNone)
End Sub
...
con.BeginTrans
For i = 1 To .rows - 1
   If validPhoto(RetPhoto_I(grid1.TextMatrix(i, 2))) And (Val(grid1.TextMatrix(i, grid1.Cols - 1)) <> 0) Then
        ...
        con.Execute addInsert(aInsert, "FILE4_20")
        aInsert = AddFlag(Empty, "DATE_PRINT", addDate(Format(Date, "YYYY-MM-DD HH:NN")))
        If grid1.TextMatrix(i, 1) = "" Then
            con.Execute addUpdate(aInsert, "FILE2_10", "FILE2_10.CODE = " & grid1.TextMatrix(i, 0))
        Else
            con.Execute addUpdate(aInsert, "FILE2_11", "FILE2_11.MEMBER = " & grid1.TextMatrix(i, 0) & " AND FILE2_11.CODE = " & grid1.TextMatrix(i, 1))
        End If
    End If
Next
con.CommitTrans
...
Dim sDate As String
For i = 1 To grid1.rows - 1
    If (validPhoto(RetPhoto_I(grid1.TextMatrix(i, 2)))) And (Val(grid1.TextMatrix(i, grid1.Cols - 1)) <> 0) Or True Then
        nCard = nCard + 1
        nCol = IIf(nCol = nCols, 1, nCol + 1)
        nRow = IIf(nCol = 1, nRow + 1, nRow)
        nRow = IIf(nRow > nRows, 1, nRow)
...
'        tCard.Update
        ...
        For i2 = 1 To 1
             ...
             tCard!FontSize = 12
             tCard!text = TurnValue(IIf(Trim(grid1.TextMatrix(i, 5)) = "", ": «·«”„", grid1.TextMatrix(i, 5)))
             tCard!CardNo = nCard
             tCard.Update
             ...
             tCard.AddNew
             tCard!Right = MyMeasure(0.2) + MyMeasure(1.2)
             tCard!Top = MyMeasure(0.2) + MyMeasure(nDiffer * 2) - MyMeasure(0.1)
             tCard!Width = MyMeasure(8)
             tCard!Height = 0
             tCard!FontName = "Arial"
             tCard!FontBold = True
             tCard!ForeColor = vbBlack
             tCard!FontSize = 12
             tCard!text = TurnValue(IIf(grid1.TextMatrix(i, 1) = "", grid1.TextMatrix(i, 3), grid1.TextMatrix(i, 4)))
             ...
             tCard.AddNew
             tCard!Right = MyMeasure(0.2) + MyMeasure(1.2)
             tCard!Top = MyMeasure(0.2) + MyMeasure(nDiffer * 3)
             tCard!Width = MyMeasure(8)
             tCard!Height = 0
             tCard!FontName = "Arial"
             tCard!FontBold = True
             ...
             tCard!FontSize = 12
             tCard!text = TurnValue(IIf(grid1.TextMatrix(i, 6) = "1" Or grid1.TextMatrix(i, 6) = "", grid1.TextMatrix(i, 8), grid1.TextMatrix(i, 7)))
            If grid1.ValueMatrix(i, 11) <> 0 Then
                tCard!FontSize = 12
            ...
            End If
             ...
             tCard.AddNew
             tCard!Right = MyMeasure(2.2) + MyMeasure(1.3)
             tCard!Top = MyMeasure(0.2) + MyMeasure(nDiffer * 4)
            ...
            tCard!ForeColor = vbBlack
            tCard!FontSize = 11
            sDate = IIf(myFormat(grid1.TextMatrix(i, 9)) < myFormat(DateAdd("yyyy", 1, Date)), Format(grid1.TextMatrix(i, 9), "yyyy/m/d"), myFormat_p(DateAdd("yyyy", 1, Date)))
            tCard!text = "Ì‰ ÂÌ ›Ì " & sDate
            tCard!TextAlign = taRightTop1
            tCard!CardNo = nCard
            tCard.Update
...
' «·Ã“¡ «·«Ì”—
        ...
        tCard!text = RetPhoto_I(grid1.TextMatrix(i, 2))
        tCard!isPhoto = True
        tCard!CardNo = nCard
        tCard.Update
    End If
...
For i = 1 To grid1.rows - 1
    ...
    If Not bNoCheck Then
        aPrint = Printed_I(.TextMatrix(i, 0), .TextMatrix(i, 1), xSeason.text, con)
        
        grid1.TextMatrix(i, 10) = myFormat_p(retFlag(aPrint, "date"))
        If IsDate(grid1.TextMatrix(i, 10)) Then
...
cString = "SELECT FILE2_10.*,FILE2_10.DATE_END,TYPE_CODES.DESCA AS TYPE_DESCA,FILE2_10.TITLE" & _
...
If xPrinted.Value = 1 Then
    cString = cString & turn(cString) & "(dbo.f_printed_i(FILE2_10.code,NULL,NULL) IS NULL)"
End If

cString = cString & " ORDER BY FILE2_10.DESCA"
...
Do Until loctable.EOF
    ...
    If .FindRow(loctable!code, , 0) = -1 Then
        ...
        .TextMatrix(grid1.rows - 1, 8) = "⁄÷ÊÌ… „ﬁ”ÿ…"
        .TextMatrix(grid1.rows - 1, 9) = myFormat_p(loctable!DATE_END)
...
cString = "SELECT FILE2_11.Code,FILE2_10.DATE_END,FILE2_11.Relation,FILE2_11.Member,FILE2_11.Title,FILE2_11.descA,FILE2_10.DESCA as Desca_Member,Relation_codes.Desca as Desca_relation,CASE WHEN FILE2_11.ISMEMBER = 1 THEN TYPE_CODES.DESCA ELSE '⁄÷Ê “«∆—' END AS TYPE_DESCA " & _
          " From FILE2_11 Inner Join FILE2_10 On FILE2_11.Member = FILE2_10.Code INNER join relation_codes on FILE2_11.relation = relation_codes.code LEFT JOIN TYPE_CODES ON FILE2_10.TYPE = TYPE_CODES.CODE"
...
If xAppend.text = "" Then
    ...
    End If
Else
    cString = cString & turn(cString) & "FILE2_11.MEMBER = " & xcode1.text & " and FILE2_11.code = " & xAppend.text
End If

If xPrinted.Value = 1 Then
    cString = cString & turn(cString) & "(dbo.f_printed_i(FILE2_11.member,FILE2_11.code,NULL) IS NULL)"
End If
...
Do Until loctable.EOF
    ...
    If .FindRow(loctable!MEMBER & "-" & loctable!code, , 2) = -1 Then
        ...
        .TextMatrix(.rows - 1, 7) = loctable!Desca_relation & ""
         
        .TextMatrix(grid1.rows - 1, 8) = IIf(loctable!relation = "1", "⁄÷ÊÌ… „ﬁ”ÿ…", "⁄÷Ê  «»⁄")
        .TextMatrix(grid1.rows - 1, 9) = myFormat_p(loctable!DATE_END)
        .TextMatrix(grid1.rows - 1, .Cols - 1) = -1
    End If
    loctable.MoveNext
...
Set loctable = myCmd("[dbo].[sp_paid_no_card_install]", con, adStoredProc, aPrm, 1000)
If Not (loctable.EOF And loctable.BOF) Then
    nRecordcount = loctable.RecordCount
End If
...
Do Until loctable.EOF
    ...
    Else
        ...
        .TextMatrix(.rows - 1, 6) = loctable!relation & ""
        .TextMatrix(.rows - 1, 7) = loctable!RELATION_DESCA & ""
...
With grid1
    ...
    .TextMatrix(0, 8) = "‰Ê⁄ «·⁄÷ÊÌ…"
    .TextMatrix(0, 9) = " «—ÌŒ «·«‰ Â«¡"
    .TextMatrix(0, 10) = " «—ÌŒ «Œ— ÿ»«⁄…"
    .TextMatrix(0, 11) = "»œ· ›«ﬁœ"
    .TextMatrix(0, 12) = "«Œ Ì«—"
...
For i = 0 To grid1.rows - 1
    nAll = nAll + 1
    If validPhoto(RetPhoto_I(grid1.TextMatrix(i, 2))) Then nPhoto = nPhoto + 1
Next
...
Dim aRet As Variant
aRet = GetFields("select DESCA from FILE2_10 where code = " & xcode1.text)
