...
Begin VB.Form grdpaid_bg1 
   Caption         =   "ÅÌãÇáí ÇíÕÇáÇÊ ÓÏÇÏ ÈÇÏÌÇÊ"
   ClientHeight    =   10110
   ClientLeft      =   90
   ClientTop       =   465
   ClientWidth     =   16785
...
cString = "SELECT '', FILE6_50H.DOC_NO,FILE6_50H.[NAME],CASE WHEN FILE6_50H.ENG = 1 THEN 'ãåäÏÓíä' ELSE 'ÛíÑ ãåäÏÓíä' END,FILE6_50H.UNION_NO,FILE6_50H.LIC,FILE6_50H.AUTH,FILE6_50H.PHONE, CONVERT(VARCHAR(10),FILE6_50H.DATE,111),SUM(FILE6_50.TOTAL)" & _
          "  FROM FILE6_50H INNER JOIN FILE6_50 ON FILE6_50H.DOC_NO = FILE6_50.DOC_NO"

If IsDate(xDate1.Text) Then
    cString = cString & turn(cString) & "FILE6_50H.DATE >= " & DateSq(xDate1.Text)
    aHeader(1) = BetweenString(myFormat_p(xDate1.Text), myFormat_p(xDate2.Text))
End If
          
If IsDate(xDate2.Text) Then
    cString = cString & turn(cString) & "FILE6_50H.DATE <= " & DateSq(xDate2.Text)
    aHeader(1) = BetweenString(myFormat_p(xDate1.Text), myFormat_p(xDate2.Text))
End If

If xuser.MatchedWithList Then
    cString = cString & turn(cString) & "FILE6_50H.USERCODE = " & xuser.BoundText
    aHeader(2) = "ÇáãæÙÝ : " & xuser.Text
End If

cString = cString & " group by FILE6_50H.DOC_NO,FILE6_50H.[NAME],FILE6_50H.DATE,FILE6_50H.ENG,FILE6_50H.UNION_NO,FILE6_50H.PHONE,FILE6_50H.LIC,FILE6_50H.AUTH Order by FILE6_50H.DOC_NO"
...
Dim nTotal_Sales As Double, nTotal_in As Double
    ...
    .TextMatrix(0, 3) = "ÇáÝÆÉ"
    .TextMatrix(0, 4) = "ÑÞã ÇáÞíÏ ÈÇáäÞÇÈÉ"
    .TextMatrix(0, 5) = "ÑÞã ÇáÑÎÕÉ"
    .TextMatrix(0, 6) = "ÑÞã ÇáÊæßíá"
    .TextMatrix(0, 7) = "ÇáÊáíÝæä"
    .TextMatrix(0, 8) = "ÊÇÑíÎ ÇáãÓÊäÏ"
    .TextMatrix(0, 9) = "ÞíãÉ ÇáãÓÊäÏ"
    ...
    If .rows > 1 Then
        ...
        .Subtotal flexSTSum, -1, 9, "#", &HC0FFC0, vbBlack, True, "  "
        For I = 0 To 8
