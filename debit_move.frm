...
Begin VB.Form debitMovefrm 
   Caption         =   "ÍÑßÉ ÇáÏÇÆäæä"
   ClientHeight    =   10455
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15210
   ...
   End
End
Attribute VB_Name = "DebitMovefrm"
...
If IsDate(xdate1.Text) Then
    ...
    loctable.Open "select sum(value_P - value_M) as Balance from Debit_Move where " & _
...
cString = "Select  Debit_Move.*,FILE0_50.DESCA AS BOX_DESCA from Debit_Move LEFT JOIN FILE0_50 ON DEBIT_MOVE.BOX = FILE0_50.CODE  Where DEBIT_MOVE.CODE = " & MyParn(xCode.BoundText)
...
Do
   ...
   .TextMatrix(.Rows - 1, 5) = Myvalue(DocTable!Value_P, "FIXED")
   .TextMatrix(.Rows - 1, 6) = Myvalue(DocTable!Value_M, "FIXED")
   .TextMatrix(.Rows - 1, 7) = Format(nPrevious + Val(DocTable!Value_P & "") - Val(DocTable!Value_M & ""), "FIXED")
   nPrevious = Round(nPrevious + Val(DocTable!Value_P & "") - Val(DocTable!Value_M & ""), 2)
   DocTable.MoveNext
   I = I + 1
...
cHeader1 = "ÊÝÕíáí ÍÑßÉ ÇáÏÇÆä " & xCode.Text & "  ÎáÇá ÝÊÑÉ"
...
data1.RecordSource = "FILE8_81"

Set xCode.RowSource = data1
xCode.ListField = "Desca"
xCode.BoundColumn = "code"

grid1.FormatString = "ÊÇÑíÎ |" & "ÑÞã ÇáãÓÊäÏ|" & "äæÚ |" & "ÈíÇä |" & "ÇáÎÒäÉ|" & "ãÏíæäíÉ|" & "ÓÏÇÏ ãÏíæäíÉ|" & "ÑÕíÏ"
