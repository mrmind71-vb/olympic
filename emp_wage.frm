   ...
   Begin VB.Frame Frame8 
      ...
      Begin Threed.SSCommand cmdLast 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "emp_wage.frx":9A37
         Caption         =   "«ŒÌ—"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "emp_wage.frx":BC07
      End
      Begin Threed.SSCommand cmdNext 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "emp_wage.frx":DD4F
         Caption         =   "·«Õﬁ "
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "emp_wage.frx":FF17
      End
      Begin Threed.SSCommand cmdPrevious 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "emp_wage.frx":12066
         Caption         =   "”«»ﬁ"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "emp_wage.frx":14246
      End
      Begin Threed.SSCommand cmdFirst 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "emp_wage.frx":163A1
         Caption         =   "√Ê·"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "emp_wage.frx":1855D
      End
   End
...
Dim oSearchDoc As New Search3, oSearchCharge As New Search3, oSearchEmp As New Search3
...
With grid1
    For i = IIf(Row = -1, 1, Row) To IIf(nRow = -1, grid1.Rows - 2, Row)
        ...
        aInsert = AddFlag(aInsert, "CODE", addstring(grid1.TextMatrix(i, 3)))
        aInsert = AddFlag(aInsert, "Desca", addstring(grid1.TextMatrix(i, 5)))
        aInsert = AddFlag(aInsert, "[VALUE_M]", Val(grid1.TextMatrix(i, 6)))
        aInsert = AddFlag(aInsert, "[VALUE_P]", Val(grid1.TextMatrix(i, 7)))
...
If ActiveControl.Name = grid1.Name Then
    If grid1.Col = 1 Then
        grid1.TextMatrix(grid1.Row, 1) = oSearchCharge.grid1.TextMatrix(oSearchCharge.grid1.Row, 0)
        grid1.TextMatrix(grid1.Row, 2) = oSearchCharge.grid1.TextMatrix(oSearchCharge.grid1.Row, 1)
        Grid1_AfterEdit grid1.Row, grid1.Col
        Unload oSearchCharge
        CellPos 13, grid1.Row, grid1.Col
    ElseIf grid1.Col = 3 Then
        grid1.TextMatrix(grid1.Row, 3) = oSearchEmp.grid1.TextMatrix(oSearchEmp.grid1.Row, 0)
        grid1.TextMatrix(grid1.Row, 4) = oSearchEmp.grid1.TextMatrix(oSearchEmp.grid1.Row, 1)
        Grid1_AfterEdit grid1.Row, grid1.Col
        Unload oSearchEmp
        CellPos 13, grid1.Row, grid1.Col
    End If
...
cString = "SELECT " & cFileHeader & ".Doc_No, Convert(Varchar," & cFileHeader & ".Date,111),Min(" & DocClient & ".Desca)" & _
          " FROM (" & cFileHeader & " inner join " & cFile & " on " & cFileHeader & ".doc_no = " & cFile & ".Doc_NO) left Join " & DocClient & " on " & cFile & ".CODE = " & DocClient & ".Code"
...
If Row = grid1.Rows - 1 Then
    MyAddItem
ElseIf Row = grid1.Rows - 2 And (Col = 0 Or Col = 1 Or Col = 5) Then
    MyEditItem grid1, Row, Col, Col = 1
    Calctotals
End If
...
If grid1.Col = 2 Or grid1.Col = 4 Then grid1.Editable = flexEDNone Else grid1.Editable = flexEDKbdMouse
End Sub
...
'        Exit Function
'    End If
    If Val(.TextMatrix(i, 4 + 2)) = 0 And Val(.TextMatrix(i, 5 + 2)) = 0 Then
        MsgBox "«·ﬁÌ„… €Ì— „”Ã·…"
        Exit Function
    End If
...
With grid1
    cString = "SELECT FILE8_90.[BOX],FILE8_90.CHARGE,FILE8_51.DESCA,FILE8_90.CODE,DRIVER.DESCA,FILE8_90.DESCA,CASE WHEN VALUE_M = 0 THEN NULL ELSE VALUE_M END,CASE WHEN VALUE_P = 0 THEN NULL ELSE VALUE_P END,FILE8_90.[ID] " & _
               " FROM FILE8_90 INNER JOIN DRIVER ON FILE8_90.CODE = DRIVER.CODE LEFT JOIN FILE8_51 ON FILE8_90.CHARGE = FILE8_51.CODE " & _
               " WHERE FILE8_90.Doc_no = " & MyParn(xDoc_No.Text)
...
ElseIf KeyCode = 112 And grid1.Col = 3 Then
    DriverLookupAll Me, oSearchEmp, "driver = 0"
...
ElseIf Col = 3 Then
    ...
    Else
        ...
        aRet = GetFields("SELECT CODE,DESCA FROM DRIVER WHERE CODE = " & MyParn(grid1.EditText) & " AND DRIVER = 0", con)
        If IsEmpty(aRet) Then
           MsgBox "«·ﬂÊœ €Ì— ’ÕÌÕ"
           Cancel = True
        Else
    ...
    End If
ElseIf Col = 4 + 2 Then
    If Val(grid1.EditText) <> 0 And Val(grid1.TextMatrix(Row, 5 + 2)) <> 0 Then
        grid1.TextMatrix(Row, 5 + 2) = ""
    End If
ElseIf Col = 5 + 2 Then
    If Val(grid1.EditText) <> 0 And Val(grid1.TextMatrix(Row, 4 + 2)) <> 0 Then
        grid1.TextMatrix(Row, 4 + 2) = ""
    End If
End If
End Sub
...
searchArray = Array(Generalarray, listarray, GrdArray)
oSearchCharge.Caption = "≈” ⁄·«„ "
oSearchCharge.Show 1
End Sub
Private Function Calctotals()
Dim nTotal As Double
With grid1
For i = 1 To grid1.Rows - 2
    nTotal = nTotal + Round(Val(grid1.TextMatrix(i, 4 + 2)), 2)
Next
...
Private Sub Fixgrd()
With grid1
.FormatString = "Œ“‰…|" & "«·„’—Ê›|" & "»Ì«‰ «·„’—Ê›|" & "«·„ÊŸ›|" & "«·«”„|" & "«·»Ì«‰|" & "„”ÕÊ»« |" & "«Ìœ«⁄« |"
.ColWidth(0) = 1500
.ColWidth(1) = 800
.ColWidth(2) = 2000
.ColWidth(3) = 800
...
With grid1
If Not (IsNumeric(.TextMatrix(nRow, 4 + 2)) Or IsNumeric(.TextMatrix(nRow, 5 + 2))) Then Exit Function
If (Not IsNumeric(.TextMatrix(nRow, 3))) Then Exit Function
...
KeyCode = 0
If Col < grid1.Cols - 2 - IIf(Col = 4 + 2 And Val(grid1.TextMatrix(Row, 4 + 2)) <> 0, 1, 0) Then
    grid1.Col = Col + 1 + IIf(Col = 1 Or Col = 3, 1, 0)
