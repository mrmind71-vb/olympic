...
If nType = 1 Or nType = 2 Then
    ...
    Do Until rsYears.EOF
        If rsYears.RecordCount = 1 Then
            cInsert = cInsert & addPaidItems(sDoc_no, pCode, rsYears!CODE, nYear_code, pType, pDate, 0, cWhere_rel, pCon) & ";"
        ...
        ElseIf Not rsYears!NO_LATE Then
            I = I + 1
            If pType = 2 Then
                cInsert = cInsert & addPaidItems(sDoc_no, pCode, rsYears!CODE, nYear_code, 1, pDate, I, cWhere_rel, pCon) & ";"
            Else
                cInsert = cInsert & addPaidItems(sDoc_no, pCode, rsYears!CODE, nYear_code, 1, pDate, I, cWhere_rel, pCon) & ";"
            End If
        Else
            cInsert = cInsert & addPaidItems(sDoc_no, pCode, rsYears!CODE, nYear_code, 1, pDate, 0, cWhere_rel, pCon) & ";"
        End If
        rsYears.MoveNext
    Loop
Else
...
If cInsert <> "" Then
    ...
    cInsert = cInsert & ";" & "update file6_20h set years_desca = dbo.f_get_years(" & sDoc_no & ") where file6_20h.doc_no = " & sDoc_no & ";"
    addPayment = AddFlag(addPayment, "sql", cInsert)
