...
Begin VB.Form fix_codesfrm 
   ...
   End
End
Attribute VB_Name = "fix_codesfrm"
...
cString = "SELECT FIX_CODES.CODE as [«·ﬂÊœ],FIX_CODES.DESCA as [«·»Ì«‰],case when FIX_CODES.[MONTHES] = 0 then NULL else FIX_CODES.[MONTHES] end  as [„œ… «·«’·«Õ »«·«‘Â—],case when FIX_CODES.KILOES = 0 then null else FIX_CODES.KILOES end AS [„”«›… «·«’·«Õ »«·ﬂÌ·Ê „ —]" & _
          " FROM FIX_CODES"
If Trim(xDescA.Text) <> "" Then
    cString = cString & turn(cString) & MyParnAnd(xDescA.Text, "FIX_CODES.desca")
End If
cString = cString & " order by FIX_CODES.CODE"
...
If Col = 1 Then
    ...
    Else
        ...
        cString = "Select code from FIX_CODES where desca = " & MyParn(grid1.EditText)
...
If KeyCode = 46 And grid1.Row <> grid1.Rows - 1 Then
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Õ–› !! „Ê«›ﬁ", vbOKCancel + vbDefaultButton2) = vbOK Then
            con.BeginTrans
            On Error GoTo myerror
            con.Execute "delete from FIX_CODES where CODE = " & MyParn(grid1.TextMatrix(grid1.Row, 0))
            con.CommitTrans
...
aInsert = AddFlag(aInsert, "[MONTHES]", Val(grid1.TextMatrix(Row, 2)))
aInsert = AddFlag(aInsert, "[KILOES]", Val(grid1.TextMatrix(Row, 3)))
con.BeginTrans
On Error GoTo myerror
If grid1.TextMatrix(Row, 0) = "" Then
    Dim sCode As String
    sCode = Newflag("FIX_CODES", "CODE", con)
    aInsert = AddFlag(aInsert, "[CODE]", sCode)
    con.Execute addInsert(aInsert, "FIX_CODES")
Else
    con.Execute addUpdate(aInsert, "FIX_CODES", "FIX_CODES.CODE = " & grid1.TextMatrix(Row, 0))
End If
con.CommitTrans
myreplace = True
Exit Function
