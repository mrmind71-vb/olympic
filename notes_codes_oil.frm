...
Begin VB.Form notes_codes_oilfrm 
   ...
   Begin VB.CommandButton CmdExit 
      ...
      Left            =   2835
      MaskColor       =   &H00FFFFFF&
      Picture         =   "notes_codes_oil.frx":0000
      RightToLeft     =   -1  'True
      Style           =   1  'Graphical
      TabIndex        =   4
      TabStop         =   0   'False
   ...
   End
End
Attribute VB_Name = "notes_codes_oilfrm"
...
cString = "SELECT NOTES_CODES_OIL.CODE as [«·ﬂÊœ],NOTES_CODES_OIL.DESCA as [—ﬁ„ «·œ› —],NOTES_CODES_OIL.BON AS [«Ê· »Ê‰],NOTES_CODES_OIL.BON_COUNT AS [⁄œœ «·»Ê‰« ]," & _
          " FROM NOTES_CODES_OIL"
If Trim(xDesca.Text) <> "" Then
    cString = cString & turn(cString) & MyParnAnd(xDesca.Text, "NOTES_CODES_OIL.desca")
End If
If Val(xbons.Text) <> 0 Then
    cString = cString & turn(cString) & "BON <= " & Val(xBon.Text) & " AND " & "BON_LAST >= " & Val(xBon.Text)
End If

cString = cString & " ORDER BY NOTES_CODES_OIL.CODE"
...
Set notes_codes_oilfrm = Nothing
Err.Clear
End Sub
...
'        cString = "Select code from NOTES_CODES_OIL where desca = " & MyParn(grid1.EditText)
...
If KeyCode = 46 And grid1.Row <> grid1.Rows - 1 Then
    If Trim(grid1.TextMatrix(grid1.Row, 0)) <> "" Then
        If MsgBox("Õ–› !! „Ê«›ﬁ", vbOKCancel + vbDefaultButton2) = vbOK Then
            con.BeginTrans
            On Error GoTo myerror
            con.Execute "delete from NOTES_CODES_OIL where CODE = " & MyParn(grid1.TextMatrix(grid1.Row, 0))
            con.CommitTrans
...
If grid1.TextMatrix(Row, 0) = "" Then
    Dim sCode As String
    sCode = Newflag("NOTES_CODES_OIL", "CODE", con)
    aInsert = AddFlag(aInsert, "[CODE]", sCode)
    con.Execute addInsert(aInsert, "NOTES_CODES_OIL")
Else
    con.Execute addUpdate(aInsert, "NOTES_CODES_OIL", "NOTES_CODES_OIL.CODE = " & grid1.TextMatrix(Row, 0))
End If
con.CommitTrans
myreplace = True
Exit Function
