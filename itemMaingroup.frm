...
Begin VB.Form itemsMainGroupFrm 
   ...
   End
End
Attribute VB_Name = "itemsMainGroupFrm"
...
cString = "SELECT CODE, DESCA,[SECTION],CODE AS [ID]" & _
          " FROM FILE1_50G WHERE ISRAW = " & nRaw
...
If grid1.TextMatrix(Row, grid1.Cols - 1) = "" Then
    con.Execute CreateInsert(aInsert, "FILE1_50G")
    grid1.TextMatrix(Row, grid1.Cols - 1) = grid1.TextMatrix(grid1.Row, 0)
Else
    con.Execute CreateUpdate(aInsert, "FILE1_50G", " WHERE FILE1_50G.code = " & MyParn(grid1.TextMatrix(Row, grid1.Cols - 1)))
End If
con.CommitTrans
Exit Sub
myerror:
...
ElseIf grid1.Col <> 0 And grid1.TextMatrix(grid1.Row, 0) = "" Then
    grid1.Editable = flexEDNone
Else
...
openCon con
clist1 = StrList("Select code,desca from file1_10SC Order by code")
