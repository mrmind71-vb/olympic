...
Begin VB.Form documentsfrm 
   ...
   End
End
Attribute VB_Name = "documentsfrm"
...
On Error GoTo myerror
cSource = Document_Files(sCode, xPhoto.Caption)
MyCreateFolder Checks_Dir(sCode)
...
If MyPrinter.ShowPrinter(Me.hwnd) Then
    If MyPrinter.Range = iprAllPages And grid1.rows - 1 > 0 Then
        For i = 1 To grid1.rows - 2
            If validPhoto(Document_Files(sCode, sCode & "-" & grid1.TextMatrix(i, grid1.Cols - 1))) Then
                imgx2.Import.FromFile Document_Files(sCode, sCode & "-" & grid1.TextMatrix(i, grid1.Cols - 1))
            End If
...
On Error GoTo myerror
imgx1.Images.Clear
If Not MyCreateFolder(Checks_Dir(sCode)) Then Exit Sub
If xPhoto.Caption <> "" Then
    imgx1.Import.FromFile Document_Files(sCode, xPhoto.Caption)
End If
Exit Sub
myerror:
Err.Clear
...
With grid1
    ...
    For i = IIf(Row = -1, 1, Row) To IIf(Row = -1, grid1.rows - 2, Row)
        ...
        If grid1.TextMatrix(i, .Cols - 1) = "" Then
            con.Execute addInsert(aInsert, "FILE1_10_D")
        Else
            con.Execute addUpdate(aInsert, "FILE1_10_D", "ID = " & .TextMatrix(i, .Cols - 1))
        End If
    Next
    con.CommitTrans
End With
...
cString = "SELECT DESCA as [«·«”„],NULL ,NULL,NULL,FILE1_10_D.ID" & _
          " FROM FILE1_10_D"
cString = cString & turn(cString) & "FILE1_10_D.CODE = " & MyParn(sCode)
cString = cString & " order by FILE1_10_D.ROW"
...
'    camerafrm.sPicture = Document_Files(sCode, xPhoto.Caption)
'    camerafrm.Show 1
    LoadPhoto
...
ElseIf Col = 4 Then
    If MsgBox("Õ–› «·’Ê—…", vbOKCancel + vbDefaultButton2) Then
        fs.DeleteFile Document_Files(sCode, xPhoto.Caption)
        LoadPhoto
    End If
End If
Exit Sub
...
If grid1.TextMatrix(grid1.Row, grid1.Cols - 1) = "" Then
    xPhoto.Caption = ""
Else
    xPhoto.Caption = sCode & "-" & grid1.TextMatrix(grid1.Row, grid1.Cols - 1)
End If
End Sub
...
ElseIf KeyCode = 46 Then
    If Trim(grid1.TextMatrix(grid1.Row, grid1.Cols - 1)) <> "" Then
        If MsgBox("«·€«¡ «·”Ã· ?? Â· «‰  „ √ﬂœ", vbYesNo + vbDefaultButton2) = vbYes Then
            con.BeginTrans
            On Error GoTo myerror
            con.Execute "DELETE  FROM FILE1_10_D  where id = " & grid1.TextMatrix(grid1.Row, grid1.Cols - 1)
            If fs.FileExists(Document_Files(sCode, sCode & "-" & grid1.TextMatrix(grid1.Row, grid1.Cols - 1))) Then
                fs.DeleteFile Document_Files(sCode, sCode & "-" & grid1.TextMatrix(grid1.Row, grid1.Cols - 1))
            End If
            con.CommitTrans
            myreplace
            myLoadGrd
