...
cField = cField & turn(cField, " + ") & _
    myiif(cWhere & turn(cWhere, " And ") & " (TYPE = '1')", "[VALUE_M]") & " AS FIRST_BAL"
...
cField = cField & "," & _
         myiif(cWhere, "[VALUE_M]- [VALUE_P] ") & " AS LAST_BAL"

With grid1 '                           0                           1
    cString = "  select FILE8_101.CODE, FILE8_101.DESCA, " & _
                 cField & _
                " FROM CREDIT_MOVE INNER JOIN FILE8_101 ON CREDIT_MOVE.CODE = FILE8_101.CODE"
    If Trim(xDesca.Text) <> "" Then cString = cString & turn(cString) & MyParnAnd(xDesca.Text, "FILE8_101.DESCA")
    If IsDate(xDate2.Text) Then
        cString = cString & turn(cString) & "CREDIT_MOVE.DATE <= " & DateSq(xDate2.Text)
    End If
    
    cString = cString & " GROUP BY FILE8_101.DESCA , FILE8_101.CODE "
