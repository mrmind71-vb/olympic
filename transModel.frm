...
If xDoc_No.Enabled Then
    xDoc_No.Text = RetZero(Val(Newflag("FILE1_70H", "doc_no")))
    aInsert(0, 1) = addstring(xDoc_No.Text)
    con.Execute CreateInsert(aInsert, "FILE1_70H")
Else
    con.Execute CreateUpdate(aInsert, "FILE1_70H", " where doc_no = " & addstring(xDoc_No.Text))
End If
myreplaceGrd
con.CommitTrans
myreplace = True
...
cString = "DELETE  FROM FILE1_70 WHERE DOC_NO = " & MyParn(xDoc_No.Text) & " AND MODEL = " & MyParn(xModel.Text)
con.Execute cString
With grid10
    For nRow = 3 To .Rows - 1
        For nCol = 2 To .Cols - 1
            If IsNumeric(.TextMatrix(nRow, nCol)) Then
                cString = "Insert into FILE1_70 (doc_no,Item,Model,Quant)" & _
                           "Values(" & _
...
If MsgBox("Õ–› «·„” ‰œ »«·ﬂ«„·  ?, Â· «‰  „Ê«›ﬁ ø", 1 + 256) = vbOK Then
    On Error GoTo myerror
    con.BeginTrans
    con.Execute "Delete From FILE1_70 where Doc_No = " & MyParn(xDoc_No.Text)
    con.Execute "Delete From FILE1_91 where Doc_No = " & MyParn(xDoc_No.Text)
    con.Execute "Delete From FILE1_70H where Doc_No = " & MyParn(xDoc_No.Text)
    con.CommitTrans
    CardTable.Requery
...
Generalarray(1) = "SELECT FILE1_70H.Doc_No, CONVERT(VARCHAR(10),FILE1_70H.[Date],111),FILE0_40.DESCA,FILE0_40_1.DESCA,NOTES " & _
                  " FROM (FILE1_70H INNER JOIN FILE0_40 ON FILE1_70H.STORE1 = FILE0_40.CODE) INNER JOIN FILE0_40 AS FILE0_40_1 ON FILE1_70H.STORE2 = FILE0_40_1.CODE "
Generalarray(2) = " order by FILE1_70H.Doc_No"
Generalarray(3) = 4000
Generalarray(5) = False

listarray(0, 0) = " «—ÌŒ-«”„ «·„ÊœÌ·"
listarray(0, 1) = "##FILE1_70H.Date## " & _
                  " Or (DOC_NO in (Select DOC_NO FROM FILE1_70 INNER JOIN FILE1_10H ON FILE1_70.MODEL = FILE1_10H.MODEL WHERE %%FILE1_10H.DESCA%%))"
...
CardTable.Open "SELECT * FROM FILE1_70H ORDER BY DOC_NO", con, adOpenStatic, adLockReadOnly, adCmdText
...
If KeyCode = 46 And grid1.Row <> 0 Then
    If MsgBox("Õ–› «·’‰› „‰ «·„” ‰œ ?, Â· «‰  „Ê«›ﬁ ø", 1 + 256) = vbOK Then
        ...
        If Grid2.TextMatrix(Grid2.Row, 0) <> "" Then
            cString = "DELETE  FROM FILE1_70 WHERE DOC_NO = " & MyParn(xDoc_No.Text) & " AND MODEL = " & MyParn(Grid2.TextMatrix(Grid2.Row, 0))
            con.Execute cString
        End If
        con.CommitTrans
        myDefineGrdModel
...
With grid1
    cString = "SELECT FILE1_70.ITEM,FILE1_70.QUANT,FILE1_70.COST  " & _
               " FROM FILE1_70 " & _
...
cString = "SELECT FILE1_70.MODEL,FILE1_10H.DESCA,SUM(Quant),SUM(FILE1_70.QUANT * FILE1_70.COST)  " & _
           " FROM FILE1_70  INNER JOIN FILE1_10H ON FILE1_70.MODEL = FILE1_10H.MODEL" & _
           " where FILE1_70.Doc_no = " & MyParn(xDoc_No.Text) & _
           " GROUP BY FILE1_70.MODEL,FILE1_10H.DESCA"
...
xDoc_No.Text = RetZero(Newflag("FILE1_70H", "DOC_NO"))
xDate.Text = ""
xOrderNo.Text = ""
xMosm.Text = ""
xStore1.BoundText = ""
