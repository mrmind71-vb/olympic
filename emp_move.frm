...
Begin VB.Form empmovefrm 
   Caption         =   "ÍÑßÉ ÇáÚÇãáíä"
   ClientHeight    =   10455
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   15210
...
If IsDate(xdate1.Text) Then
    ...
    loctable.Open "select sum(PLUS - MINUS) as Balance from emp_move " & _
                  " WHERE CODE = " & MyParn(xCode.BoundText) & _
    ...
    If nPrevious <> 0 Then
        ...
        grid1.TextMatrix(grid1.Rows - 1, 2) = "ÑÕíÏ ÓÇÈÞ"
...
cString = "Select  emp_move.*,FILE0_50.DESCA AS BOX_DESCA from emp_move LEFT JOIN FILE0_50 ON emp_move.BOX = FILE0_50.CODE  Where emp_move.CODE = " & MyParn(xCode.BoundText)
...
cString = cString & " Order by [Date],PLUS,MINUS"
...
Do
   ...
   .TextMatrix(.Rows - 1, 5) = Myvalue(DocTable!PLUS, "FIXED")
   .TextMatrix(.Rows - 1, 6) = Myvalue(DocTable!MINUS, "FIXED")
   .TextMatrix(.Rows - 1, 7) = Format(nPrevious + Val(DocTable!PLUS & "") - Val(DocTable!MINUS & ""), "FIXED")
   nPrevious = Round(nPrevious + Val(DocTable!PLUS & "") - Val(DocTable!MINUS & ""), 2)
   DocTable.MoveNext
   i = i + 1
...
cHeader1 = "ÊÝÕíáí ÍÑßÉ ÇáÚÇãá " & xCode.Text & "  ÎáÇá ÝÊÑÉ"
...
data1.RecordSource = "SELECT * FROM DRIVER WHERE DRIVER = 0"
Set xCode.RowSource = data1
xCode.ListField = "Desca"
xCode.BoundColumn = "code"

grid1.FormatString = "ÊÇÑíÎ |" & "ÑÞã ÇáãÓÊäÏ|" & "äæÚ |" & "ÈíÇä |" & "ÇáÎÒäÉ|" & "ãÓÊÍÞÇÊ|" & "ãÞÈæÖÇÊ ãä ÇáÔÑßÉ|" & "ÑÕíÏ"
...
If KeyCode = 112 Then DriverLookupAll Me, oSearch, "Driver = 0"
End Sub
...
If Not xCode.MatchedWithList Then
    xCode.BoundText = RetZero(xCode.Text)
