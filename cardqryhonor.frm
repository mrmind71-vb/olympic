...
Begin VB.Form cardqryHonorfrm 
   ...
   Begin VSFlex7Ctl.VSFlexGrid grid1 
      Bindings        =   "cardqryhonor.frx":EF88
      Height          =   6000
      Left            =   90
      TabIndex        =   39
      Top             =   2070
   ...
   End
End
Attribute VB_Name = "cardqryHonorfrm"
...
If KeyAscii = 13 Then
    ...
    If Val(unMyCodeBar(xCode.Text, "2")) <> 2 Then
...
If KeyCode = 112 Then
    MemberHonorLookupAll Me, oSearch
End If
End Sub
...
SettingArray(cUpMargin) = MyMeasure(1.2) + MyMeasure(Val(xDown.Text) / 10)
...
For i = 1 To grid1.rows - 1
    If validPhoto(RetPhotoh(grid1.TextMatrix(i, 0))) And (Val(grid1.TextMatrix(i, grid1.Cols - 1)) <> 0) Then
        ...
        nspace = 1
        nspaceFixed = 0.7
        nspace = nspace + IIf(grid1.TextMatrix(i, 4) = "", 0.4, 0)
        ...
        tCard.AddNew
        tCard!Right = MyMeasure(0.5) + MyMeasure(0.8)
        ...
        tCard!fontsize = 12
        tCard!Text = turn(grid1.TextMatrix(i, 3), grid1.TextMatrix(i, 3) & " : ") & grid1.TextMatrix(i, 1)
...
'        tCard.Update
        ...
        If grid1.TextMatrix(i, 4) <> "" Then
            nRowItem = nRowItem + 1
            tCard!Right = MyMeasure(0.5) + MyMeasure(1.4)
            tCard!Top = MyMeasure(0) + MyMeasure(nspace * nRowItem)
            tCard!Width = MyMeasure(8)
            tCard!Height = 0
            tCard!FontName = "Arial"
            tCard!FontBold = True
            tCard!ForeColor = &H424F32
            tCard!fontsize = 12
...
cString = "SELECT FILE2_10.*,SHARE_CODES.DESCA AS SHARE_DESCA FROM FILE2_10 INNER JOIN SHARE_CODES ON FILE2_10.SHARE = SHARE_CODES.CODE"

If xShare.MatchedWithList Then
    cString = cString & turn(cString) & "FILE2_10.SHARE = " & xShare.BoundText
End If
...
Do Until GRDTABLE.EOF
    ...
    If bAddRow Then
        ...
        .TextMatrix(grid1.rows - 1, 2) = "⁄÷ÊÌ… ›Œ—Ì…"
