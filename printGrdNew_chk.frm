    ...
    If Not grid1.RowHidden(nRow) Then
        For nCol = nBegin To nEnd Step nStep
            If Not grid1.ColHidden(nCol) Then
                ...
                Else
                    If bLeft Then
                        sb.Append grid1.Cell(flexcpTextDisplay, nRow, nCol) & "|"
                    Else
                        If grid1.ColDataType(nCol) = flexDTBoolean And nRow > grid1.FixedRows - 1 Then
                            sb.Append IIf(Val(grid1.TextMatrix(nRow, nCol)) = 0, "", "‰⁄„") & "|"
                        ElseIf grid1.ColDataType(nCol) = flexDTDouble And nRow > grid1.FixedRows - 1 Then
                            sb.Append grid1.Cell(flexcpTextDisplay, nRow, nCol) & "|"
                        Else
                            sb.Append ArbString(grid1.Cell(flexcpTextDisplay, nRow, nCol)) & "|"
                        End If
                    End If
                End If
            End If
        Next
        sb.Append Tr(sb.GetAsString, ";")
    End If
Next
...
.AddTable cFormat, cHeader, sb.GetAsString(), , , True
