...
Begin VB.Form report_insfrm4 
   Caption         =   "«⁄÷«¡ „ﬁ”ÿÌ‰ Õ”»  «—ÌŒ ‰Â«Ì… «·⁄÷ÊÌ…"
   ClientHeight    =   3600
   ClientLeft      =   1065
   ClientTop       =   1875
   ClientWidth     =   4800
   ...
   End
End
Attribute VB_Name = "report_insfrm4"
...
If IsDate(xdate_begin1.text) Then
    aHeader(1) = " «—ÌŒ ‰Â«Ì… «·⁄÷ÊÌ… „‰ " & BetweenString(xdate_begin1.text, xDate_Begin2.text)
    cWhere = cWhere & turn(cWhere, " and ") & "FILE2_10.DATE_END >= " & DateSq(xdate_begin1.text)
End If

If IsDate(xDate_Begin2.text) Then
    aHeader(1) = " «—ÌŒ ‰Â«Ì… «·⁄÷ÊÌ… „‰ " & BetweenString(xdate_begin1.text, xDate_Begin2.text)
    cWhere = cWhere & turn(cWhere, " and ") & "FILE2_10.DATE_END <= " & DateSq(xDate_Begin2.text)
End If
...
Else
    ...
    End If
    cmdStatus.Tag = cmdStatus.Tag & turn(cmdStatus.Tag, ",") & MyParn(oSearchStatus.grid1.TextMatrix(oSearchStatus.grid1.Row, 0))
End If
oSearchStatus.Hide
End Sub
