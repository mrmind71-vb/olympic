...
Begin VB.Form paidfrm3 
   ...
   Begin VB.CommandButton cmdTransData 
      Caption         =   "Command1"
      Height          =   825
      Left            =   90
      RightToLeft     =   -1  'True
   ...
   Begin VB.Frame Frame8 
      ...
      Begin Threed.SSCommand cmdLast 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "PAID3.frx":BE61
         Caption         =   "«ŒÌ—"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "PAID3.frx":E031
      End
      Begin Threed.SSCommand cmdNext 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "PAID3.frx":10179
         Caption         =   "·«Õﬁ "
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "PAID3.frx":12341
      End
      Begin Threed.SSCommand cmdPrevious 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "PAID3.frx":14490
         Caption         =   "”«»ﬁ"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "PAID3.frx":16670
      End
      Begin Threed.SSCommand cmdFirst 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Picture         =   "PAID3.frx":187CB
         Caption         =   "√Ê·"
         Alignment       =   4
         PictureAlignment=   9
         PictureDisabledFrames=   1
         PictureDisabled =   "PAID3.frx":1A987
      End
   End
...
If xDoc_No.Tag = DefineMode Then
    ...
    xForm_no.Text = Newflag(cFileHeader, "FORM_NO", con, "SEASON = " & sSeason_Student)
    aInsert = AddFlag(aInsert, "DOC_NO", addvalue(xDoc_No.Text))
    aInsert = AddFlag(aInsert, "FORM_NO", addvalue(xForm_no.Text))
    aInsert = AddFlag(aInsert, "SEASON", sSeason_Student)
...
With grid1
    For i = IIf(Row = -1, 1, Row) To IIf(Row = -1, grid1.rows - 2, Row)
        ...
        If grid1.TextMatrix(i, grid1.Cols - 1) = "" Then
            con.Execute addInsert(aInsert, "FILE6_40")
        Else
            con.Execute addUpdate(aInsert, "FILE6_40", "ID = " & grid1.TextMatrix(i, .Cols - 1))
        End If
    Next
End With
End Sub
...
aMember = GetFields("select FILE3_10.*,DEGREE_CODES.[GROUP] AS GROUP_CODE FROM FILE3_10 LEFT JOIN DEGREE_CODES ON FILE3_10.DEGREE = DEGREE_CODES.CODE  WHERE FILE3_10.CODE = " & xCode.Text, con)
If IsEmpty(aMember) Then Exit Function
If Not IsNumeric(retFlag(aMember, "GROUP_CODE") & "") Then Exit Function

cString = "select TOP 1 file3_20.* from file3_20 where DEGREE = " & retFlag(aMember, "GROUP_CODE") & " ORDER BY CODE"
aret = GetFields(cString, con)
If IsEmpty(aret) Then Exit Function

cString = "SELECT FILE6_40.DOC_NO FROM FILE6_40 INNER JOIN FILE6_40H ON FILE6_40.DOC_NO = FILE6_40H.DOC_NO"
cString = cString & turn(cString) & "FILE6_40.MEMBER = " & xCode.Text & " AND FILE6_40.CODE = " & retFlag(aret, "CODE")
cString = cString & turn(cString) & "FILE6_40H.SEASON = " & sSeason_Student
cString = cString & turn(cString) & "FILE6_40.DOC_NO <> " & MyParn(xDoc_No.Text)
...
If NewItem(retFlag(aret, "CODE")) Then
    ...
    grid1.TextMatrix(grid1.rows - 1, 4) = IIf(retFlag(aMember, "FIRST"), 0, retFlag(aret, "VALUE"))
    grid1.AddItem ""
End If
AddMember = True
End Function
...
Dim aMember As Variant
aMember = GetFields("select * from FILE3_10 WHERE CODE = " & xCode.Text, con)

If IsEmpty(aMember) Then Exit Function
cString = "select TOP 1 FILE3_20.* from FILE3_20 where isCard = 1 ORDER BY CODE"
aret = GetFields(cString, con)
...
aMember = GetFields("select * from FILE3_10 WHERE CODE = " & xCode.Text, con)
If IsEmpty(aMember) Then Exit Function
cString = "select TOP 1 FILE3_20.* from FILE3_20 where isDamage = 1 ORDER BY CODE"
aret = GetFields(cString, con)
If Not IsEmpty(aret) Then
    If NewItem(retFlag(aret, "CODE")) Then
        grid1.TextMatrix(grid1.rows - 1, 0) = retFlag(aret, "CODE")
...
cString = "select TOP 1 FILE3_20.* from FILE3_20 where isNew = 1 ORDER BY CODE"
aret = GetFields(cString, con)
If Not IsEmpty(aret) Then
...
cString = "SELECT  FILE6_40H.DOC_NO,CONVERT(VARCHAR(10),FILE6_40H.DATE,111), FILE3_10.DESCA,CASE WHEN USERS.DESCA IS NULL THEN FILE6_40H.USERNAME ELSE USERS.DESCA END" & _
          "  FROM  FILE6_40H INNER JOIN FILE3_10 ON FILE6_40H.CODE = FILE3_10.CODE LEFT JOIN USERS ON FILE6_40H.USERCODE = USERS.CODE"

Generalarray(1) = cString
Generalarray(2) = " ORDER BY FILE6_40H.DATE,FILE6_40H.Doc_No"
Generalarray(3) = 6000
Generalarray(5) = False

listarray(0, 0) = "«·«”„- «—ÌŒ «·„” ‰œ-«·«”„"
listarray(0, 1) = "(%%FILE3_10.Desca%% or **FILE6_40H.DOC_NO** OR" & _
                  " ##FILE6_40.Date##)"

listarray(1, 0) = "«·ﬂÊœ"
listarray(1, 1) = "(**FILE6_40H.CODE**)"

listarray(2, 0) = "«·„” Œœ„"
listarray(2, 1) = "(%%USERS.DESCA%%  OR %%FILE6_40H.USERNAME%%)"
...
myUndo
End Sub
Private Sub cmdTransData_Click()
Dim loctable As ADODB.Recordset, nRecord As Long
Set loctable = New ADODB.Recordset
loctable.Open "SELECT FILE6_40H.*,FILE3_10.DESCA AS CODE_DESCA FROM FILE6_40H INNER JOIN FILE3_10 ON FILE6_40H.CODE = FILE3_10.CODE WHERE FILE6_40H.SEASON < " & sSeason_Student, con, adOpenStatic, adLockReadOnly, adCmdText
Dim aInsert As Variant
con.BeginTrans
On Error GoTo myError
Do Until loctable.EOF
    nRecord = nRecord + 1
    Me.Caption = nRecord
    aInsert = AddFlag(Empty, "DOC_NO", addvalue(loctable!doc_no))
    aInsert = AddFlag(aInsert, "FORM_NO", addvalue(loctable!doc_no))
    ...
    aInsert = AddFlag(aInsert, "[DESCA]", addstring(loctable!code_Desca))
    aInsert = AddFlag(aInsert, "[USERNAME]", addstring(loctable!UserName & ""))
    aInsert = AddFlag(aInsert, "[USERNAME2]", addstring(loctable!UserName2 & ""))
    aInsert = AddFlag(aInsert, "[TIME]", addDate(loctable!Time & ""))
    aInsert = AddFlag(aInsert, "[TIME2]", addDate(loctable!Time2 & ""))
    con.Execute addInsert(aInsert, "FILE6_40H_OLD")
    loctable.MoveNext
Loop
...
Set loctable = New ADODB.Recordset
loctable.Open "SELECT FILE6_40.*,FILE3_10.DESCA AS DESCA_RELATION,FILE3_20.DESCA AS DESCA_CODE FROM FILE6_40 INNER JOIN FILE6_40H ON FILE6_40.DOC_NO = FILE6_40H.DOC_NO INNER JOIN FILE3_20 ON FILE6_40.CODE = FILE3_20.CODE LEFT JOIN FILE3_10 ON FILE6_40.MEMBER = FILE3_10.CODE  WHERE FILE6_40H.SEASON < " & sSeason_Student, con, adOpenStatic, adLockReadOnly, adCmdText
On Error GoTo myError
Do Until loctable.EOF
    aInsert = AddFlag(Empty, "DOC_NO", addstring(loctable!doc_no & ""))
    aInsert = AddFlag(aInsert, "CODE", addvalue(loctable!CODE & ""))
    aInsert = AddFlag(aInsert, "MEMBER", addvalue(loctable!Member & ""))
    aInsert = AddFlag(aInsert, "DESCA_CODE", addstring(loctable!DESCA_CODE & ""))
    aInsert = AddFlag(aInsert, "DESCA_RELATION", addstring(loctable!Desca_relation & ""))
    aInsert = AddFlag(aInsert, "VALUE", Val(loctable!Value & ""))
    aInsert = AddFlag(aInsert, "NOTES", addstring(loctable!Notes))
    con.Execute addInsert(aInsert, "FILE6_40_OLD")
    loctable.MoveNext
Loop
con.CommitTrans
...
With grid1
cString = "SELECT FILE6_40.CODE,FILE3_20.DESCA,FILE6_40.MEMBER,FILE3_10.DESCA,FILE6_40.VALUE,FILE6_40.NOTES,FILE6_40.[ID] " & _
           " FROM FILE6_40 INNER JOIN FILE3_20 ON FILE6_40.CODE = FILE3_20.CODE LEFT JOIN FILE3_10 ON FILE6_40.MEMBER = FILE3_10.CODE" & _
           " WHERE FILE6_40.Doc_no = " & MyParn(xDoc_No.Text)
...
If KeyCode = 112 Then
    StudentLookupAll Me, oSearchMember
End If
End Sub
...
Dim aret As Variant
aret = GetFields("select DESCA from FILE3_10 where code = " & xCode.Text)
If Not IsEmpty(aret) Then
...
Private Sub Fixgrd()
With grid1
'cString = "SELECT FILE6_40.CODE,FILE3_20.DESCA,FILE6_40.MEMBER,FILE6_40.DESCA,FILE6_40.VALUE,FILE6_40.NOTES,FILE6_40.[ID] " & _
...
If ValidInt(xCode.Text) Then
    aret = GetFields("SELECT DESCA,VALUE,ISCARD,RELATION FROM FILE3_20 where CODE = " & sItem)
    If IsEmpty(aret) Then Exit Function
...
cString = "SELECT FILE6_40.DOC_NO,FILE6_40H.FORM_NO,FILE6_40H.DATE,CASE WHEN USERS.DESCA IS NULL THEN FILE6_40H.USERNAME ELSE USERS.DESCA END AS USER_NAME,FILE6_40H.DATE2,FILE6_40H.CODE AS CODE_MEMBER,FILE3_10.DESCA AS DESCA_MEMBER, FILE6_40.CODE,FILE3_20.DESCA AS ITEM_DESCA,FILE6_40.DESCA,DEGREE_GROUP_CODES.DESCA AS GROUP_DESCA," & _
          "FILE6_40.VALUE,FILE6_40.[NOTES]" & _
          " FROM FILE6_40 INNER JOIN FILE6_40H ON FILE6_40.DOC_NO = FILE6_40H.DOC_NO " & _
          " INNER JOIN FILE3_10 ON FILE6_40H.CODE = FILE3_10.CODE" & _
          " INNER JOIN FILE3_20 ON FILE6_40.CODE = FILE3_20.CODE" & _
          " INNER JOIN DEGREE_CODES ON FILE3_10.DEGREE = DEGREE_CODES.CODE" & _
          " INNER JOIN DEGREE_GROUP_CODES ON DEGREE_CODES.[GROUP] = DEGREE_GROUP_CODES.CODE" & _
          " LEFT JOIN USERS ON FILE6_40H.USERCODE = USERS.CODE"
cString = cString & turn(cString) & "FILE6_40.DOC_NO = " & xDoc_No.Text

Dim aTotal As Variant
aTotal = GetFields("Select sum(FILE6_40.total) as total from FILE6_40 where doc_no = " & xDoc_No.Text)
...
Do Until loctable.EOF
    ...
    temptable!str21 = "«Ì’«· ”œ«œ " & loctable!GROUP_DESCA
    temptable!val1 = 1
...
For i = 1 To grid1.rows - 1
    If grid1.TextMatrix(i, 2) <> "" Then
        If Not validPhoto(RetPhoto_s(grid1.TextMatrix(i, 2))) Then grid1.Cell(flexcpForeColor, i, 0, i, .Cols - 1) = vbRed
    End If
Next
End With
End Sub
...
xDoc_No.Text = GetField("select top 1 doc_no from file6_40h where form_no = " & xForm_no.Text & " and season = " & xSeason.Caption)
If xDoc_No.Text = "" Then xDoc_No.Text = GetField("select top 1 doc_no from file6_40h where form_no = " & xForm_no.Text)
xDoc_No_LostFocus
End Sub
