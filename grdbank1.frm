...
If xBank.BoundText <> "" Then aHeader(1) = "«·»‰ﬂ : " & xBank.Text
...
DATA1.RecordSource = "Select Code,DescA From FILE5_10"
...
If IsDate(xdate1.Text) Then
    cString = "select sum([value1]  - [value2] ) as Balance from bankmove"
    cString = cString & turn(cString) & " bank = " & MyParn(xBank.BoundText)
    cString = cString & turn(cString) & " [DATE] < " & DateSq(xdate1.Text)
    nPrevious = Round(Val(GetDesca(cString)), 2)
    If nPrevious <> 0 Then
        .AddItem ""
        .TextMatrix(.Rows - 1, 0) = "—’Ìœ ”«»ﬁ"
        .TextMatrix(.Rows - 1, 1) = "—’Ìœ ”«»ﬁ"
        .TextMatrix(.Rows - 1, 2) = Myvalue(nPrevious, "FIXED")
        .TextMatrix(.Rows - 1, 4) = Myvalue(nPrevious, "FIXED")
    End If
End If

cString = "Select typeDesca,Desca,sum(Value1) as sumofValue1,sum(Value2) as sumOfValue2  from BankMove"
cString = cString & turn(cString) & "BANK = " & MyParn(xBank.BoundText)
...
End If
cString = cString & " GROUP BY typeDesca,Desca,TYPE"
cString = cString & " Order by TYPE"
Dim loctable As New ADODB.Recordset
loctable.Open cString, con, adOpenStatic, adLockReadOnly, adCmdText
Do Until loctable.EOF
    .AddItem ""
    .TextMatrix(.Rows - 1, 0) = loctable!TypeDesca & ""
    .TextMatrix(.Rows - 1, 1) = loctable!desca & ""
    .TextMatrix(.Rows - 1, 2) = Myvalue(Val(loctable!sumOfvalue1 & ""), "FIXED")
    .TextMatrix(.Rows - 1, 3) = Myvalue(Val(loctable!sumOfValue2 & ""), "FIXED")
    .TextMatrix(.Rows - 1, 4) = Myvalue(nPrevious + Val(loctable!sumOfvalue1 & "") - Val(loctable!sumOfValue2 & ""), "FIXED")
    nPrevious = Val(.TextMatrix(.Rows - 1, 4))
    loctable.MoveNext
Loop
End With
loctable.Close
